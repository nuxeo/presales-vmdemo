...
then
	...
	echo "It seems there are no custom templates deployed on your system, you may want to run the script 03-updateBackupTemplatesFromWeb first (it's not mandatory)"
...
${NUXEO_SERVER_DIR}/bin/nuxeoctl mp-remove ${NUXEO_DEFAULT_PACKAGE_TO_REMOVE} --accept=true|| exit 1
...
then
	...
	echo "No instance.clid found, the hotfixes will not be installed on the server. Altough not mandatory, it is highly recommended. The instance.clid is looked for in the parent of the nuxeo_demo folder. In your case in:"
